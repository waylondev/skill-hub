...
function Show-Help {
    Write-Host "Skill-Hub CLI - 把 Confluence 的理论知识，变成 AI 帮你干活的能力"
    Write-Host ""
    Write-Host "Usage: skill command [args]"
    Write-Host ""
    Write-Host "Commands:"
    Write-Host "  search keyword   - 搜索 Skill"
    Write-Host "  install name    - 安装 Skill 到本地"
    Write-Host "  uninstall name  - 卸载本地 Skill"
    Write-Host "  run name        - 执行 Skill"
    Write-Host "  list            - 列出所有 Skill"
